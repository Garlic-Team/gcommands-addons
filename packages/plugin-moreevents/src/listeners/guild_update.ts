...
new Listener({
	...
	run: (oldGuild, newGuild) => {
		...
		if (oldGuild.maximumMembers !== newGuild.maximumMembers) {
			client.emit('guildMaximumMembersUpdate', newGuild, oldGuild.maximumMembers, newGuild.maximumMembers);
		}

		if (oldGuild.partnered !== newGuild.partnered) {
			client.emit('guildPartnerUpdate', newGuild, oldGuild.partnered, newGuild.partnered);
		}

		if (oldGuild.verified !== newGuild.verified) {
			client.emit('guildVerifyUpdate', newGuild, oldGuild.verified, newGuild.verified);
		}
	},
});
